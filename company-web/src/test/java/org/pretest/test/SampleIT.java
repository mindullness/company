...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "company-ear.ear");

        // Current directory is the root of the "company-web" project. Go up one level
        // and enter the "company-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../company-ejb/target/company-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "company-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/company-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "company-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("org.pretest.test");
